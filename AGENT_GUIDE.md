...
1. **승인 없는 코드 수정 절대 금지:** 아무리 사소한 디버깅, 오타 수정, 1줄짜리 코드 변경이라도 반드시 `implementation_plan.md`를 통해 변경안(계획)을 먼저 작성하고 대표님(사용자)의 명시적 승인(Approval)을 받아야 합니다. 승인 전에 코드를 수정하는 것은 엄격히 금지됩니다.
...
3. **승인 체크리스트 필수화:** 코드를 수정하기 전, 모든 에이전트는 `task.md` 최상단에 `[ ] 대표님 변경안 승인 완료` 체크 항목을 만들고, 이것이 `[x]` 상태가 아닐 경우 절대로 코드 수정 도구를 호출해서는 안 됩니다.
4. **배포 실행은 대표님 전담:** 에이전트는 `git` 또는 `publish.bat`을 직접 실행하지 않는다. 작업을 완료하면 배포 여부를 되묻지 말고, 대표님이 `C:\Users\Won` 상위 폴더에서 그대로 실행할 수 있는 `publish.bat "배포문"` 한 줄을 항상 제공한다. 이 방식은 두 대의 컴퓨터 간 변경사항 동기화를 위한 고정 운영 절차다.
...
|---|---|---|
| **정체** | 두뇌 — 기획·초안·외부연결 | 손 — 코딩·실행·검증 |
| **주요 작업** | 스펙 문서, 코드 초안, SQL 스크립트, UI 목업, 외부 API 탐색, 비즈니스 문서(PPTX·DOCX) | 파일 통합, 로컬 실행·테스트, 디버깅, 배포문 작성 |
| **파일 쓰기** | `Q_Platform/` 하위 어디든 직접 저장 가능 | 동일 |
| **배포** | **직접 실행하지 않음** — 완료 시 배포문 제공 | **직접 실행하지 않음** — 완료 시 배포문 제공 |
| **git 명령** | **직접 호출 금지** | **직접 호출 금지** |
...
### Rule 2. 배포는 대표님이 상위 폴더의 `publish.bat`으로 실행

- 에이전트는 Git 명령(`git add`, `git commit`, `git push`)과 `publish.bat`을 직접 호출하지 않는다.
- 실제 스크립트 위치는 `C:\Users\Won\publish.bat`이다.
- 작업 완료 시 에이전트는 `publish.bat "한 줄 변경 요약"` 형식의 배포문을 최종 답변에 반드시 제공한다.
- 대표님은 `C:\Users\Won` 상위 폴더에서 해당 명령을 직접 실행한다.
- 매 작업마다 배포 여부를 다시 묻지 않는다. 배포문을 바로 제공하고 실제 실행 여부는 대표님이 결정한다.
- 이유: 두 대의 컴퓨터에서 작업하므로, 루트 저장소와 정책 미러의 동기·인증·로그를 `publish.bat` 한 곳에서 통합하기 위함이다.
...
5. 검증 후 사용자가 실행할 `publish.bat "배포문"` 제공
```

### C. 변경 사항 GitHub 배포문 제공

```cmd
publish.bat "한 줄 변경 요약"
```

- 위 명령은 대표님이 `C:\Users\Won`에서 직접 실행한다.
- 에이전트는 명령을 실행하거나 배포 여부를 질문하지 않는다.
- 구현·검증을 마친 모든 작업의 최종 답변에 변경 내용에 맞는 배포문을 포함한다.
...
- **식별자 역할을 분리한다.** `account_id`는 인증·외래키·권한·트랜잭션을 위한 시스템 내부 식별자로만 사용하고, 회원에게 표시하거나 기억·입력·공유하도록 요구하지 않는다. 사용자 화면·초대 링크·프로필 URL·운영 검색·대외 문서에는 Q-ID를 사용한다.
- Q-ID는 `이름이니셜 3자리 + 전 회원 고유순번 8자리`의 유일한 공개 신원이다. 이름이니셜은 정정될 수 있지만 8자리 순번은 평생 중복되지 않고 변경되지 않는다. 서버는 대외 입력 Q-ID를 현재 Q-ID 또는 `q_id_aliases`로 확인한 뒤 내부 `account_id`로 해석한다.
- DB 관계는 `account_id` 외래키로 안전하게 연결하되, 감사·운영 기록에는 사건 당시 Q-ID를 함께 남긴다. 일반 API와 관리자 화면은 Q-ID를 입력·표시하고 서버 내부에서만 `account_id`로 변환한다.
- 통합인증 세션은 010Q 인증 본체가 관리하고, 각 서비스에는 짧게 만료되는 일회용 인증코드로 서비스 전용 세션을 발급한다. `.q.co.kr` 전체에 하나의 공용 쿠키를 배포하는 방식은 기본안으로 사용하지 않는다.
- 베타 인증 기본안은 **카카오 OAuth 계정 실검증과 고유 사용자 ID 연결 → 패스키(지문·얼굴·화면 잠금)로 일상 로그인**이다. 카카오 전화번호와 가입서 번호의 일치는 베타 가입 차단 조건으로 사용하지 않으며, 기존 비밀번호는 전환기 비상수단으로만 유지한다.
...
- 관리자 지정은 이름·전화번호가 아니라 불변 `account_id`로만 하며, 관리자 등록 절차에서 발급된 전용 패스키만 인정한다. 인증 없는 관리자 백도어는 금지하고, 기기 분실 시 다른 관리자의 10분짜리 일회용 승인 링크 또는 본인의 오프라인 1회용 복구코드를 사용한다.
...
### Q. "GitHub 에 올려주세요"
A. 에이전트는 직접 올리지 않고 `publish.bat "변경 요약"` 배포문을 제공한다. 대표님이 `C:\Users\Won`에서 실행한다.

### Q. "에이전트가 직접 결제·계약·배포 진행 가능?"
A. **불가.** 모든 외부 약정·결제·배포 실행은 사용자(서대원) 전담이다. 에이전트는 배포문까지만 작성한다.
...
| 에이전트가 `publish.bat` 직접 실행 | 두 컴퓨터 동기화 절차는 대표님이 상위 폴더에서 직접 통제 |
| 헌법선언문 무단 변경 | 상위 규범 |
...
**문서 끝.**
*Q. v1.3 AGENT_GUIDE — 2026-07-19 · 사용자 직접 publish.bat 실행 및 에이전트 배포문 제공 절차 확정*
